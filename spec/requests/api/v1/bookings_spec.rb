...
RSpec.describe Api::V1::BookingsController, type: :controller do
  ...
  let(:resort) { Resort.create(name: 'Resort Name', address: '123 Main St.', description: 'A beautiful resort', phone_number: 1_234_567_890, email: 'resort@example.com', country: 'USA', city: 'New York', available_rooms: 10, max_occupancy: 20, base_price: 100, user_id: user.id) }
  let(:valid_attributes) {
    { reservation_date: Date.today, returning_date: Date.today + 1.week, selected_city: "New York", user_id: user.id, resort_id: resort.id }
  }
