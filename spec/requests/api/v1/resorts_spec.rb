...
RSpec.describe Api::V1::ResortsController, type: :controller do
  ...
  let(:valid_attributes) {
    { name: "Resort Name", address: "123 Main St.", description: "A beautiful resort", phone_number: 1234567890, email: "resort@example.com", country: "USA", city: "New York", available_rooms: 10, max_occupancy: 20, base_price: 100, user_id: user.id }
  }

  let(:invalid_attributes) {
    { name: nil, address: nil, description: nil, phone_number: nil, email: nil, country: nil, city: nil, available_rooms: nil, max_occupancy: nil, base_price: nil, user_id: nil }
  }
  ...
  describe "POST #create" do
    ...
    context "with invalid params" do
      it "renders a JSON response with errors for the new resort" do
        ...
        expect(JSON.parse(response.body)).to include("name", "address", "description", "phone_number", "email", "country", "city", "available_rooms", "max_occupancy", "base_price", "user_id")
      end
    end
  end
